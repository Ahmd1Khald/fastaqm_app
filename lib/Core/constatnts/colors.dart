import 'package:flutter/material.dart';

class MyColors {
  static const Color myPurble = Color.fromRGBO(68, 39, 158, 1);
  static const Color myLightPurble = Color.fromRGBO(156, 126, 248, 1);
  static const Color myBlue = Color.fromRGBO(30, 144, 255, 1);

  static const Color primaryColor = Color(0xff9B7EF8);
  static const Color orange = Color(0xffEF9A53);
  static const Color babyOrange = Color(0xffF5D5AE);
  static const Color darkBlue = Color(0xff1D2356);
}
