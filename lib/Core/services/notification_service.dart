...
// Calculate the next instance of the specified time
  ...
  Future<void> fridayNotification({
    required int hour,
    required int minutes,
    required int id,
    required String title,
    ...
    // Check if the notification has been used before
    bool notificationUsed = await getFridayNotificationUsedStatus(id);
    print("Friday notification ++++++++");

    if (!notificationUsed) {
      // Calculate the next instance of the specified time
      final tz.TZDateTime scheduledDate =
          _nextFridayInstanceOfTime(hour, minutes);

      print(scheduledDate);

      // Save the scheduled time to shared preferences
      await saveScheduledTime(id, scheduledDate);
      ...
      // Mark the notification as used
      await setFridayNotificationUsedStatus(id);
    } else {
      print("Notification already used");
    }
  }

  Future<bool> getFridayNotificationUsedStatus(int id) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    return prefs.getBool('notification_used_friday_$id') ?? false;
  }

  Future<void> setFridayNotificationUsedStatus(int id) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setBool('notification_used_friday_$id', true);
  }

  Future<tz.TZDateTime> getScheduledTime(int id) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    final int scheduledMillis = prefs.getInt('scheduled_time_$id') ?? 0;
  ...
  Future<void> saveScheduledTime(int id, tz.TZDateTime scheduledDate) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setInt(
        'scheduled_time_$id', scheduledDate.millisecondsSinceEpoch);
  }

  tz.TZDateTime _nextFridayInstanceOfTime(int hour, int minutes) {
    ...
    // If the scheduled time is before the current time, add a week
    if (scheduledDate.isBefore(now)) {
